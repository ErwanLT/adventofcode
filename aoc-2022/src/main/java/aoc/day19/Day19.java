...
public class Day19 implements Day {
    ...
    public String part1(List<String> input) {
        var sum = zip(getQuality(Integer.MAX_VALUE, 24, 100000, input), rangeClosed(1, 999).boxed(), Pair::new)
                .mapToLong(p -> p.a() * p.b())
                .sum();
    ...
    private Stream<Long> getQuality(int limit, int minutes, int capacity, List<String> input) {
        List<Input> l = input.stream().map(s -> readString(s, "Blueprint %n: Each ore robot costs %n ore. Each clay robot costs %n ore. Each obsidian robot costs %n ore and %n clay. Each geode robot costs %n ore and %n obsidian.", " ", Input.class)).limit(limit).toList();
