...
public class Day18 implements DayOld {
    ...
    public String part1(List<String> input) {
        ...
        return String.valueOf(magnitude(in.get(0)));
    }

    @Override
    public String part2(List<String> input) {
        List<String> in = input.stream().filter(e -> !e.isEmpty()).map(this::reduce).collect(Collectors.toCollection(ArrayList::new));
        int magnitude = IntStream.range(0, in.size()).boxed().flatMap(x -> IntStream.range(0, in.size()).filter(y -> x!=y).mapToObj(y -> new Point(x, y))).mapToInt(p -> magnitude(reduce("["+in.get(p.x)+","+in.get(p.y)+"]"))).max().getAsInt();
