...
public class Day06 extends Day2019 {
    ...
    public Object part1() {
        ...
        for (Map.Entry<String, Collection<String>> entry : orbits.asMap().entrySet()) {
    ...
    public List<String> findOrbit(String orbitValue) {
        return orbits.asMap().entrySet().stream().filter(e -> e.getValue().contains(orbitValue)).map(Map.Entry::getKey).collect(Collectors.toCollection(ArrayList::new));
    }
}
